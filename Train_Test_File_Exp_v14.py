...
def expFile(filename, POS_File, model):
    with open(filename) as file:
        ...
        #NER_Value = []
        train_labels_dict = {}
        index_model_olan_0 = 0
        index_modelde_olmayan_0 = 0
        for line in lines:
            #print(line)
            if (len(line) != 8 ):
                continue
            ...
            for NER_Word_Splitted in line[7].split(' '):
                if (NER_Word_Splitted in model.vocab):
                    index_model_olan_0 = index_model_olan_0 + 1
                    if (line[4] == 'Thing'):
                        value4Thing.append(model[NER_Word_Splitted])
                    elif (line[4] == 'Organization'):
                        value4Organization.append(model[NER_Word_Splitted])
                    elif (line[4] == 'Person'):
                        value4Person.append(model[NER_Word_Splitted])
                    elif (line[4] == 'Location'):
                        value4Location.append(model[NER_Word_Splitted])
                    elif (line[4] == 'Product'):
                        value4Product.append(model[NER_Word_Splitted])
                    elif (line[4] == 'Event'):
                        value4Event.append(model[NER_Word_Splitted])
                    elif (line[4] == 'Character'):
                        value4Character.append(model[NER_Word_Splitted])
                else:
                    index_modelde_olmayan_0 = index_modelde_olmayan_0 + 1
                    #print(' Olmayan NER_Word_Splitted')
                    #print(NER_Word_Splitted)
        print('index_model_olan_0')
        print(index_model_olan_0)
        print('index_modelde_olmayan_0')
        print(index_modelde_olmayan_0)
        value4Thing = np.array(value4Thing)
        value4Organization = np.array(value4Organization)
        value4Person = np.array(value4Person)
        value4Location = np.array(value4Location)
        value4Product = np.array(value4Product)
        value4Event = np.array(value4Event)
        value4Character = np.array(value4Character)
        ...
        data = {
            ...
            #'2 W Cons Vow Ratio': [''],
            ##'2 W POS Tag': [''],
            '2 W Cos Sim  With Person Avg': [''],
            '2 W Cos Sim  With Thing Avg': [''],
            '2 W Cos Sim  With Organization Avg': [''],
            '2 W Cos Sim  With Location Avg': [''],
            '2 W Cos Sim  With Product Avg': [''],
            '2 W Cos Sim  With Event Avg': [''],
            '2 W Cos Sim  With Character Avg': ['']
        ...
        for j in range(df.index.size):
            ...
            if (df.index[j] == '& Start Doc &' or df.index[j] == '& Start Tweet &' or df.index[j] == '& End Tweet &' or df.index[j] == '& End Doc &' or df.index[j] is None):
                ...
                df.iloc[j]['2 W Cos Sim  With Person Avg'] = 0
                df.iloc[j]['2 W Cos Sim  With Thing Avg'] = 0
                df.iloc[j]['2 W Cos Sim  With Organization Avg'] = 0
                df.iloc[j]['2 W Cos Sim  With Location Avg'] = 0
                df.iloc[j]['2 W Cos Sim  With Product Avg'] = 0
                df.iloc[j]['2 W Cos Sim  With Event Avg'] = 0
                df.iloc[j]['2 W Cos Sim  With Character Avg'] = 0
            else:
                #print('Word: ')
                #print(df.index[j])
                ...
                if (temp_index in model.vocab):
                    index_model_olan = index_model_olan + 1
                    df.iloc[j]['2 W Cos Sim  With Person Avg'] = 1 - spatial.distance.cosine(average4Person, model[temp_index])
                    #print(1 - spatial.distance.cosine(average4Person, model[temp_index]))
                else:
                    index_modelde_olmayan = index_modelde_olmayan + 1
                    df.iloc[j]['2 W Cos Sim  With Person Avg'] = 0
                if (temp_index in model.vocab):
                    index_model_olan = index_model_olan + 1
                    df.iloc[j]['2 W Cos Sim  With Thing Avg'] = 1 - spatial.distance.cosine(average4Thing, model[temp_index])
                    #print(1 - spatial.distance.cosine(average4Thing, model[temp_index]))
                else:
                    index_modelde_olmayan = index_modelde_olmayan + 1
                    df.iloc[j]['2 W Cos Sim  With Thing Avg'] = 0
                if (temp_index in model.vocab):
                    index_model_olan = index_model_olan + 1
                    df.iloc[j]['2 W Cos Sim  With Organization Avg'] = 1 - spatial.distance.cosine(average4Organization, model[temp_index])
                    #print(1 - spatial.distance.cosine(average4Organization, model[temp_index]))
                else:
                    index_modelde_olmayan = index_modelde_olmayan + 1
                    df.iloc[j]['2 W Cos Sim  With Organization Avg'] = 0
                if (temp_index in model.vocab):
                    index_model_olan = index_model_olan + 1
                    df.iloc[j]['2 W Cos Sim  With Location Avg'] = 1 - spatial.distance.cosine(average4Location, model[temp_index])
                    #print(1 - spatial.distance.cosine(average4Location, model[temp_index]))
                else:
                    index_modelde_olmayan = index_modelde_olmayan + 1
                    df.iloc[j]['2 W Cos Sim  With Location Avg'] = 0
                if (temp_index in model.vocab):
                    index_model_olan = index_model_olan + 1
                    df.iloc[j]['2 W Cos Sim  With Product Avg'] = 1 - spatial.distance.cosine(average4Product, model[temp_index])
                    #print(1 - spatial.distance.cosine(average4Product, model[temp_index]))
                else:
                    index_modelde_olmayan = index_modelde_olmayan + 1
                    df.iloc[j]['2 W Cos Sim  With Product Avg'] = 0
                if (temp_index in model.vocab):
                    index_model_olan = index_model_olan + 1
                    df.iloc[j]['2 W Cos Sim  With Event Avg'] = 1 - spatial.distance.cosine(average4Event, model[temp_index])
                    #print(1 - spatial.distance.cosine(average4Event, model[temp_index]))
                else:
                    index_modelde_olmayan = index_modelde_olmayan + 1
                    df.iloc[j]['2 W Cos Sim  With Event Avg'] = 0
                if (temp_index in model.vocab):
                    index_model_olan = index_model_olan + 1
                    df.iloc[j]['2 W Cos Sim  With Character Avg'] = 1 - spatial.distance.cosine(average4Character, model[temp_index])
                    #print(1 - spatial.distance.cosine(average4Character, model[temp_index]))
                else:
                    index_modelde_olmayan = index_modelde_olmayan + 1
                    df.iloc[j]['2 W Cos Sim  With Character Avg'] = 0
        ...
        #print(df)
        print ('index_modelde_olan')
        print(index_model_olan)
        print('index_modelde_olmayan')
        print(index_modelde_olmayan)
        print('df_train_test_arrays int cevirme oncesi')
        print(df)
        df_train_test_arrays = df.astype(float)
        print('df_train_test_arrays int cevirme sonrasi')
...
def main():
    ...
    df_train = expFile('microposts2016-neel-training_neel7.txt', 'POS_Train.txt', model)
    print('Train File Ended')
    df_Xtrain = df_train[0]
    df_Xtrain_array = df_Xtrain.values
    X = df_Xtrain_array[:, 0:7]
    Y = df_train[1]
    ...
    df_test = expFile('microposts2016-neel-test_neel7.txt','POS_Test.txt', model)
    print('Test File Ended')
    df_Xtest = df_test[0]
    df_Xtest_array = df_Xtest.values
    X = df_Xtest_array[:, 0:7]
    Y = df_test[1]
    # /Test File
